...
const batchControlSchema = z.object({
  ...
  action: z.enum(['on', 'off', 'reset'], { errorMap: () => ({ message: 'Invalid action. Must be on, off, or reset' }) }),
});

const batchParamsSchema = z.object({
  deviceIds: z.array(z.string().min(1)).min(1, 'At least one device is required'),
  params: z.record(z.union([z.number(), z.null()])).refine(
 ...
 * 批量控制设备
 */
export const batchControl = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const { deviceIds, action } = batchControlSchema.parse(req.body);
  const operator = (req as any).userId || 'system';
 ...
 * 批量更新设备参数
 */
export const batchUpdateParams = asyncHandler(async (req: Request, res: Response): Promise<void> => {
 ...
 * 批量切换设备启用状态
 */
export const batchToggleEnabled = asyncHandler(async (req: Request, res: Response): Promise<void> => {
